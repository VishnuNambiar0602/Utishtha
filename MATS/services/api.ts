...
class SupabaseBackend {
  ...
  async unassignAmbulance(tripId: string) {
    await this.initPromise;
    const trip = this.trips.find(t => t.id === tripId);
    if (!trip || !trip.ambulance_id) {
      throw new Error('Trip has no ambulance assigned');
    }
    
    await Promise.all([
      supabase.from('trips').update({ ambulance_id: null, status: TripStatus.REQUESTED }).eq('id', tripId),
      supabase.from('ambulances').update({ status: AmbulanceStatus.AVAILABLE, last_updated: new Date().toISOString() }).eq('id', trip.ambulance_id)
    ]);
    await this.refresh();
  }

  async reassignAmbulance(tripId: string, newAmbulanceId: string) {
    await this.initPromise;
    const trip = this.trips.find(t => t.id === tripId);
    if (!trip || !trip.ambulance_id) {
      throw new Error('Trip has no ambulance assigned');
    ...
    await Promise.all([
      supabase.from('trips').update({ ambulance_id: newAmbulanceId, status: TripStatus.ASSIGNED }).eq('id', tripId),
      supabase.from('ambulances').update({ status: AmbulanceStatus.AVAILABLE, last_updated: new Date().toISOString() }).eq('id', trip.ambulance_id),
      supabase.from('ambulances').update({ status: AmbulanceStatus.ENROUTE, last_updated: new Date().toISOString() }).eq('id', newAmbulanceId)
    ]);
    await this.refresh();
  }
  ...
  async addAmbulance(ambulanceData: Omit<Ambulance, 'last_updated'>) {
    await this.initPromise;
    const payload = {
      ...ambulanceData,
    ...
    if (error || !data) {
      throw new Error(error?.message || 'Failed to add ambulance');
    }
    const ambulance = this.normalizeAmbulance(data);
