-- Create medical_reports table for storing diagnostic data (v2 - On-Demand Report Generation)
-- This table stores only diagnostic data (symptoms, analysis, confidence scores)
-- PDFs are not stored - reports are generated on-demand from stored diagnostic data
-- This saves storage costs and database space while maintaining all diagnostic information
-- Run this in your Supabase SQL Editor to replace the old table

-- Drop old table if it exists (back up data first!)
-- drop table if not exists public.medical_reports cascade;

-- Create the medical_reports table (v2 - no report_url)
...
comment on table public.medical_reports is 'Stores diagnostic data for generating medical reports on-demand. No PDF files are stored.';
comment on column public.medical_reports.id is 'Unique identifier for the medical report record';
...
comment on column public.medical_reports.api_response is 'Complete JSON response from GRASP2026 diagnosis API containing all diagnostic data for report regeneration';
